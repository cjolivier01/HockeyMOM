...
def get_game_details(
    ...
    *,
    ...
    # Collect team rows
    home_team = (
        db.get_team(row["season_id"], row["division_id"], row["conference_id"], row["homeId"])
        if row.get("homeId") is not None
        else None
    )
    away_team = (
        db.get_team(row["season_id"], row["division_id"], row["conference_id"], row["awayId"])
        if row.get("awayId") is not None
        else None
    )
