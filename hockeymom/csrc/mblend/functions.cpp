...
class Flex {
 public:
  ...
  void IncrementLast32(int inc) {
    *((uint32_t*)&data[p - 4]) += inc;
  }
  ...
  uint16_t ReadBackwards16() {
    return *((uint16_t*)&data[p -= 2]);
  }
  uint32_t ReadBackwards32() {
    return *((uint32_t*)&data[p -= 4]);
  }
  uint64_t ReadBackwards64() {
    return *((uint64_t*)&data[p -= 8]);
  }
...
void ShrinkMasks(std::vector<std::shared_ptr<Flex>>& masks, int n_levels) {
  ...
  for (int l = 1; l < n_levels; ++l) {
    ...
    // first line
    input_p += Squish(
                   (uint32_t*)&masks[l - 1]->data[input_p],
                   real_lines[0],
                   in_width,
                   out_width)
        << 2;
...
#define RECORD(I, C)                                                 \
  if ((I) != current_i) {                                            \
    if (mc > 0) {                                                    \
      if (seam_map)                                                  \
        memset(&seam_map->line[x - mc], current_i, mc);              \
...
#define SEAM_DT \
  ReadSeamDT(seam_flex.get(), current_count, current_step, dt_val);
...
int CompressSeamLine(std::uint64_t* input, std::uint8_t* output, int width) {
  ...
  while (!((right_val = input[--x]) & 0xffffffff00000000) && x > 0)
    ;
  if (!(right_val & 0xffffffff00000000))
    return p;

  while (x > 0) {
    while (!((left_val = input[--x]) & 0xffffffff00000000) && x > 0)
      ;
    if (!(left_val & 0xffffffff00000000))
      break;

    if (!((right_val ^ left_val) & 0xffffffff) &&
        (step = ((std::int64_t)(right_val - left_val) >> 32) + 3) < 67 &&
        step >= 0) { // was <= 7
      if (step <= 7) {
        if (step == current_step) {
          ++current_count;
...
void SwapUnswapV(Pyramid* py, bool unswap) {
  ...
  if (height & 1) {
    ...
    if (unswap) {
      uint8_t* upper =
          (uint8_t*)(py->GetData() + ((std::int64_t)height >> 1) * py->GetPitch());
      uint8_t* lower =
          (uint8_t*)(py->GetData() + ((std::int64_t)height - 1) * py->GetPitch());
    ...
    uint8_t* upper = (uint8_t*)py->GetData();
    uint8_t* lower =
        (uint8_t*)(py->GetData() + (std::int64_t)half_height * py->GetPitch());
    for (int y = 0; y < half_height; ++y) {
      memcpy(temp, upper, byte_pitch);
