...
#define DT_MAX 0x9000000000000000
  ...
  if (!alpha || output_type == ImageType::MB_JPEG ||
      output_type == ImageType::MB_MEM /* arbitrarily have no mask */) {
    no_mask = true;
  }
  ...
  if (output_type != ImageType::MB_NONE) {
    ...
    for (int c = 0; c < 3; ++c) {
      ...
      switch (output_bpp) {
      case 8:
        output_pyramid->Out((uint8_t *)output_channels[c], width, gamma, dither,
                            true);
        break;
      case 16:
    ...
    case ImageType::MB_TIFF: {
      ...
      if (images[0]->geotiff.set) {
        ...
        Output(1, "Output georef: UL: %f %f, pixel size: %f %f\n", info.XGeoRef,
               info.YGeoRef, info.XCellRes, info.YCellRes);
