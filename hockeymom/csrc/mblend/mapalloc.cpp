...
std::unordered_map<const void*, std::unique_ptr<MapAlloc::MapAllocObject>>
    MapAlloc::object_map;
char MapAlloc::tmpdir[8192] = {
    0,
};
...
std::shared_ptr<MapAlloc::MapAllocEntry> MapAlloc::Alloc(
    std::size_t size,
    int alignment) {
 ...
 * MapAllocObject
 ***********************************************************************/
MapAlloc::MapAllocObject::MapAllocObject(std::size_t _size, int alignment)
    : size(_size) {
  ...
  if (!pointer) {
    ...
    while (true) {
      ...
      if (GetLastError() != 80) {
        ...
        FormatMessage(
            ...
            NULL);
        sprintf_s(
            filename, "Could not create temp file in %s\\: %s", tmpdir, buf);
        throw(filename);
      }
      if (suffix == 65536) {
        sprintf_s(
    ...
    map = CreateFileMapping(
        file, NULL, PAGE_READWRITE, size >> 32, size & 0xffffffff, NULL);
    if (!map) {
      sprintf_s(
          filename,
          "Could not allocate %zu temporary bytes in %s",
          size,
          tmpdir);
      throw(filename);
    }
    ...
    if (file <= 0) {
      sprintf(
          filename,
          "Could not create temp file in %s/: %s",
          tmpdir,
          strerror(errno));
      throw(filename);
    }
    ...
    if (ftruncate(file, size)) {
      ...
      sprintf(
          filename,
          "Could not allocate %zu temporary bytes in %s: %s",
          size,
          tmpdir,
          strerror(errno));
      throw(filename);
