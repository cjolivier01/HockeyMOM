				...
				for (int w = 1; w <= 2; ++w) {
					if (wrap & w) {
						...
						for (int wp = 0; wp < 2; ++wp) {
							wrap_pyramids[p]->Copy((uint8_t*)(output_pyramid->GetData() + wrap_pyramids[p]->GetX() + wrap_pyramids[p]->GetY() * (std::int64_t)output_pyramid->GetPitch()), 1, output_pyramid->GetPitch(), false, 32);
...
#define ROWS_PER_STRIP 64
		...
		void* strip = malloc((ROWS_PER_STRIP * (std::int64_t)width) * bytes_per_pixel);
		...
		for (int s = 0; s < n_strips; ++s) {
			...
			switch (output_type) {
				case ImageType::MB_TIFF: {
					TIFFWriteEncodedStrip(tiff_file, s, strip, rows * (std::int64_t)bytes_per_row);
				} break;
				case ImageType::MB_JPEG: {
