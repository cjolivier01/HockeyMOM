...
PYTHONPATH="$(pwd)/hmlib" \
  python \
  train.py mot \
  --exp_id crowdhuman_dla34 --gpus 0,1,2 --batch_size 24 --load_model '../pretrained/dla34/ctdet_coco_dla_2x.pth' --num_epochs 60 --print_iter 25 --lr_step '50' --data_cfg '../src/hmlib/cfg/crowdhuman.json' $@
cd ..
