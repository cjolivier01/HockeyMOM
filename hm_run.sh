...
# Use the installed MMCV package: its compiled ``mmcv._ext`` module lives next
# to the installed Python package.  Putting the source checkout on PYTHONPATH
# shadows that package with a source-only directory and makes the native ops
# appear to be missing.
OPENMM_PYTHONPATH="$(pwd)/openmm/mmengine:$(pwd)/openmm/mmeval:$(pwd)/openmm/mmdetection:$(pwd)/openmm/mmpose:$(pwd)/openmm/mmyolo"
