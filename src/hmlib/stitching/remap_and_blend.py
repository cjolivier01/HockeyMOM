...
def image_width(img: torch.Tensor | np.ndarray) -> int:
    if img.ndim == 2:
        return img.shape[1]
...
def image_height(img: torch.Tensor | np.ndarray) -> int:
    if img.ndim == 2:
        return img.shape[0]
    if isinstance(img, torch.Tensor):
        if img.ndim == 4:
...
def remap_video(
    ...
    while True:
        ...
        if frame_count % 20 == 0:
            print("Remapping: {:.2f} fps".format(batch_size * 1.0 / max(1e-5, timer.average_time)))
        ...
        #             make_visible_image(destination_tensor[i], enable_resizing=opts.show_scaled),
        #         )
        #         cv2.waitKey(1)
