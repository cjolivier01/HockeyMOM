...
class JDETracker(object):
    ...
    def post_process(self, dets, meta):
        ...
        #     )
        # for batch_item in range(len(dets)):
        #     for j in range(1, self.opt.num_classes + 1):
        #         dets[batch_item][j] = np.array(
        #             dets[batch_item][j], dtype=np.float32
        #         ).reshape(-1, 5)
        return dets

    # def merge_outputs(self, batched_detections):
    #     all_merged_outputs = []
    #     for batch_item in len(batched_detections):
    #         results = {}
    #         detections = batched_detections[batch_item]
    #         for j in range(1, self.opt.num_classes + 1):
    ...
    #         all_merged_outputs.append(results)
    #     return all_merged_outputs
    ...
    def detect(self, im_blob, img0, log_debug: bool = False):
        # self.frame_id += 1
        # activated_starcks = []
        # refind_stracks = []
        # lost_stracks = []
        ...
        # start_model = time.time()
        dets = self.post_process(dets=dets, meta=meta)
        duration = time.time() - start_model
        # print(f"duration: {duration} seconds")

        return dets, id_feature

        # batch_output_stracks = []
        # batched_dets = dets
    ...
    def inner_update(self, detections, id_feature, log_debug: bool = False):
        ...
        # start_model = time.time()
        dets = self.merge_outputs([detections])[1]
        # duration = time.time() - start_model
        # print(f"duration: {duration} seconds")
