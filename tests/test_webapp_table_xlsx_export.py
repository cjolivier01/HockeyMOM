...
def should_export_table_as_xlsx_freeze_identity_cols_even_when_all_other_columns_are_redacted(
    client,
):
    payload = {
        "title": "Player Stats",
        "sheet_name": "Player Stats",
        "filename": "player_stats.xlsx",
        "headers": ["Jersey #", "Player", "TOI", "Shifts"],
        "col_keys": [None, None, "toi_seconds", "shifts"],
        "rows": [["1", "Alice", "10:00", "20"]],
        "freeze_cols": 2,
    }
    resp = client.post(
        "/api/xlsx/table",
    ...
    headers = [
        str(ws.cell(row=2, column=i).value or "").replace("\n", " ").strip() for i in range(1, 5)
    ]
    headers = [h for h in headers if h]
    assert headers == ["Jersey #", "Player"]
    assert str(ws.freeze_panes) == "C3"
