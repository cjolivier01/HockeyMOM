...
resolve_python() {
  ...
  do
    ...
    if "${candidate}" - <<'PYHELPER' >/dev/null 2>&1
...
python_bin="$(resolve_python)" || {
  echo "Unable to find a python interpreter with torch and cv2 available." >&2
  exit 1
}
...
"${python_bin}" "${repo_root}/tests/test_rocm_runtime_fallbacks.py"
