...
@pytest.mark.ui
def should_not_repeat_goal_scorer_in_game_player_events_details(monkeypatch, webapp_db):
    ...
    m.HkyGameEventRow.objects.create(
        ...
        video_seconds=None,
        details="Skater (A: Helper1, Helper2)",
        attributed_players="Skater",
        attributed_jerseys="13",
        created_at=now,
        updated_at=None,
    )
    m.HkyGameEventRow.objects.create(
        ...
        game_seconds_end=None,
        video_seconds=None,
        details="Assist: Helper1 (Goal: Skater)",
        attributed_players="Helper1",
    ...
    m.HkyGameEventRow.objects.create(
        ...
        game_seconds_end=None,
        video_seconds=None,
        details="Assist: Helper2 (Goal: Skater)",
        attributed_players="Helper2",
    ...
    with _wsgi_server(app=wsgi_app) as base_url:
        with sync_playwright() as pw:
            ...
            page.locator("button.player-events-btn", has_text="Skater").click()
            ...
            metrics = page.evaluate(
                """() => {
  const table = document.querySelector("#events-drilldown-body table.events-drilldown-table");
  if (!table) return { ok: false, error: "missing drilldown table" };
  const tr = table.querySelector("tbody tr");
  if (!tr) return { ok: false, error: "missing row" };
  const tds = Array.from(tr.querySelectorAll("td"));
  const detailsTd = tds[3] || null;
  const detailsText = detailsTd ? (detailsTd.textContent || "") : "";
  return {
    ok: true,
    detailsText,
    hasScorerName: detailsText.indexOf("Skater") >= 0,
    hasA1: detailsText.indexOf("A1: #3") >= 0,
    hasA2: detailsText.indexOf("A2: #8") >= 0,
    ...
    assert not metrics["hasScorerName"], metrics
    assert metrics["hasA1"], metrics
    assert metrics["hasA2"], metrics
