...
def should_not_duplicate_goaliechange_rows_when_event_id_is_added_later(webapp_db):
    _django_orm, m = webapp_db
    now = dt.datetime.now()
    ...
    events_csv_no_id = (
        "Event Type,Team Side,Period,Game Seconds,Video Seconds\n" "Goalie Change,Away,1,100,6\n"
    )
    out1 = views._upsert_game_event_rows_from_events_csv(
        game_id=int(game.id),
        events_csv=events_csv_no_id,
    ...
    rows1 = list(
        m.HkyGameEventRow.objects.filter(
            game_id=int(game.id), event_type__key="goaliechange"
        ).values("id", "event_id", "import_key", "video_time", "video_seconds")
    )
    assert len(rows1) == 1
    assert rows1[0]["event_id"] is None
    assert rows1[0]["video_time"] == "0:06"
    assert int(rows1[0]["video_seconds"] or 0) == 6
    key1 = str(rows1[0]["import_key"])

    events_csv_with_id = (
        "Event Type,Team Side,Period,Game Seconds,Video Seconds,Event ID\n"
        "Goalie Change,Away,1,100,6,12\n"
    )
    out2 = views._upsert_game_event_rows_from_events_csv(
        game_id=int(game.id),
        events_csv=events_csv_with_id,
    ...
    assert int(rows2[0]["event_id"] or 0) == 12
    assert str(rows2[0]["import_key"]) == key1
    assert rows2[0]["video_time"] == "0:06"
    assert int(rows2[0]["video_seconds"] or 0) == 6


def should_dedupe_goaliechange_rows_with_legacy_import_keys(webapp_db):
    _django_orm, m = webapp_db
    now = dt.datetime.now()
    ...
    goalie_et = m.HkyEventType.objects.get(key="goaliechange")
    canonical = m.HkyGameEventRow.objects.get(game_id=int(game.id), event_type_id=int(goalie_et.id))
    canonical_key = str(canonical.import_key)

    m.HkyGameEventRow.objects.create(
        game_id=int(game.id),
        event_type_id=int(goalie_et.id),
    ...
    assert (
        m.HkyGameEventRow.objects.filter(
            game_id=int(game.id), event_type_id=int(goalie_et.id)
        ).count()
        == 2
    )
    ...
    rows = list(
        m.HkyGameEventRow.objects.filter(
            game_id=int(game.id), event_type_id=int(goalie_et.id)
        ).values("id", "import_key", "video_time", "video_seconds")
    )
    assert len(rows) == 1
    assert str(rows[0]["import_key"]) == canonical_key
    assert rows[0]["video_time"] == "0:06"
    assert int(rows[0]["video_seconds"] or 0) == 6
