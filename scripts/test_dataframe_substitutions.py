...
def main():
    ...
    if not has_game_dir(args.game_id):
        print(f"Game directory $HOME/Videos/{args.game_id} not found; skipping CLI runs.\n"
    ...
    base = [sys.executable, '-m', 'hmlib.cli.hmtrack', '-b=1', f'--game-id={args.game_id}', f'--config={args.config}',
            '--no-wide-start', '--no-crop', '--skip-final-video-save', f'--input-video={video}', '--no-save-video']
