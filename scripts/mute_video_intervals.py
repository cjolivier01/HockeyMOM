...
def mute_intervals(video_file, intervals, output_file):
    ...
    for i, (start, end) in enumerate(intervals):
        filter_complex_str += f'[0:a]atrim=start={start}:end={end},asetpts=PTS-STARTPTS[silent{i}];'
...
def parse_intervals(intervals_str):
    ...
    return [tuple(map(float, interval.split('-'))) for interval in intervals_str.split(',')]

def main(args: argparse.Namespace):
    # Example usage:
    #time_intervals = [(10, 20), (40, 50)]  # Replace with your start and end times in seconds

    mute_intervals(args.input_file, intervals=parse_intervals(args.intervals), output_file=args.output_file)
