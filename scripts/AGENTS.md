...
- Timestamp files for video clipping:
  - Per-player shifts: `*_video_times.txt`, `*_scoreboard_times.txt` (always written; used only for clip generation).
  - Goals: `goals_for.txt`, `goals_against.txt` (video windows).
  - Long-sheet events: `events_<Event>_<Team>_video_times.txt` and `events_<Event>_<Team>_scoreboard_times.txt`.
  - Per-player highlights (when long sheet present): `events_Goal_<player>_video_times.txt`, `events_SOG_<player>_video_times.txt`, etc.
